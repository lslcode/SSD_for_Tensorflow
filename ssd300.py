...
class SSD300:
    def __init__(self, tf_sess, isTraining):
        ...
        # default box 尺寸长宽比例
        self.box_aspect_ratio = [[1.0, 1.0, 2.0, 3.0],
    ...
    def generate_graph(self):
        ...
        # vvg16卷积层 4
        self.conv_4_1 = tf.nn.conv2d(self.conv_3_3, self.conv_weight_4_1, self.conv_strides_1, padding='SAME', name='conv_4_1')
        ...
        self.features_1 = tf.nn.conv2d(self.conv_5_3, self.features_weight_1, self.conv_strides_1, padding='SAME', name='conv_features_1')
        ...
        #self.optimizer = tf.train.NadamOptimizer(0.001)
    ...
    def run(self, input_images, actual_data):
        ...
        if self.isTraining :
            ...
            return loss_all, loss_location, loss_class
            ...
            for x in range(width):
                for y in range(height):
                    for i,ratio in zip(range(len(ratios)), ratios):
                        ...
                        if 1.0 == ratio:
                            scale = np.sqrt(scale * self.default_box_scale[index][i + 1])
                        top_x = x / float(width)
                        top_y = y / float(height)
                        box_width = scale * np.sqrt(ratio)
                        box_height = scale / np.sqrt(ratio)
    ...
    def generate_groundtruth_data(self,f_class):
        ...
        for img_index in range(input_actual_data_len):
            ...
            extract_highest_indicies(f_class[img_index], positives_count * 3)
            # 初始化负例训练数据
            for max_box_index in indicies:
                #if np.amax(gt_location[img_index][max_box_index]) == 0 and self.background_classes_val != np.argmax(f_class[img_index][max_box_index]):
                if np.amax(gt_location[img_index][max_box_index]) == 0 : 
